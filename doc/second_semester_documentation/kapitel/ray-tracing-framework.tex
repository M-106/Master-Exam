...
\label{cha:ray-tracing-framework}
	...
	Our Ray-Tracing Framework is a python package, which provides a suite of tools for simulating, analyzing, and visualizing rays (beams) in 2D images, along with general-purpose image and mathematical utilities.\\
	...
	\begin{lstlisting}[language=Python,caption=Tracing Rays using our Framework via ISM (Pseudo-Code), label=lst:example-framework-2]
		...
		ips.img.imshow(reflection_map, size=5)
	\end{lstlisting}
	...
	This algorithm works on pixel-level and can be adjusted by multiple parameters, for example the beams can be returned iterativly (with time dimension), the order of reflections can be set and bouncing of the image border can be turned on. \\
	% add ISM method
