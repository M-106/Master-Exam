% Method / Approach ?-> What name? Look how the inspire paper called it
...
\label{cha:intro}
	...
	We run all experiments on the \textbf{PhysicsGen}-Benchmark \cite{martin_spitznagel_physicsgen_2025}, which provides 2D wave-propagation simulations across urban environments. The dataset contains \textbf{19.908} training, \textbf{3.732} validation, and \textbf{1.244} test samples (for evaluation) with the official split. Each sample is available in four variations that differ in physical complexity:
	...
	Each sample consists of a 2D 256x256 gray-image representing the physical scene - specifically, the image indicates the locations of buildings, which are the main obstacles for sound propagation in urban areas – along with the corresponding simulated wave-intensity distribution from a fixed transmitter. \\
	All datapoints are collected from 3 cities in Germany \cite{martin_spitznagel_physicsgen_2025} and the ground-truth is created via NoiseModelling \cite{noisemodelling}, a physical simulation software \cite{noisemodelling}.\\
	...
	\begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt, partopsep=0pt]
		...
		\item A combined weighted loss, defined as a weighted combination of SiLog, L1, SSIM, blur, edge-aware, gradient L1, variance, and min–max range losses. The weighting coefficients vary across experiments and are specified in the corresponding experimental sections. This composite loss was introduced to better capture reflection-related effects, which are insufficiently modeled by simpler loss formulations.
	\end{itemize} 
	...
	Although this model achieved poor accuracies \cite{first_semester_report}, our previous investigation showed that this was caused by the complex-only prediction, which frequently converged to a local minimum (a nearly uniform gray image). This occurs because the target values are very close to each other \cite{first_semester_report}. Prior work introduced a loss-weighting mechanism based on the histogram, as well as additional variance and min/max losses, to mitigate this issue and improve performance.
	
	
	\subsection{Cosmos Predict 2 (Video2World)}
	We further evaluate the Cosmos Predict 2 Video2World model from NVIDIA \cite{nvidia2025cosmosworldfoundationmodel}, a large transformer-based world model originally designed for multimodal environment prediction.
	In our setup, the official PhysicsGen dataset was converted into a video by repeating the input channels along the temporal dimension using OpenCV. We employed the Google T5-11B pretrained tokenizer and applied the normalization from the official Cosmos-2 implementation.
	
	
	
	\section{Ray-Tracing Baseline}
	\label{sec:method-rays}
	To provide a physically interpretable reference for the model, we implement a lightweight 2D ray-tracing simulation \cite{img-phy-sim} which is part-wise inspired by the Digital Differential Analyzer algorithm and works direct on pixel-level. The algorithm computes x-order reflection paths without calculating the ray energies, in order to keep the complexity and computational cost low. Although much simpler than a full wave simulation, this baseline may still help to predict reflection patterns.\\
	We used third-order reflections with 36 and 360 rays as one and as multiple channels in our experiments.\\
	The selected options vary across experiments and are described in detail in the corresponding experimental sections.
	...
	The combined weighted loss is used for the complex only model and else a L1-loss is used for Pix2Pix with a small adversarial loss for sharper predictions. Cosmos Predict 2 uses its native reconstruction loss.\\
	...
	We use the same evaluate methods as from the official PhysicsGen-Benchmark:
	\begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt, partopsep=0pt]
